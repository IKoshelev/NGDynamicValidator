...
demoApp.controller("ValidateRequestEventCtrl", ["$scope", "validationEventNames",
    function ($scope,eventNames:IEventNames) {

    $scope.passport;
    $scope.foreignPassport;
    $scope.driversLicense;

        $scope.idDocumentRequired = function idDocumentRequired(value, validationInfo: IValidationInfo) {
    ...
    $scope.saveDraft = function () {
        ...
        $scope.$broadcast(eventNames.validateRequest, ["default", "blur", "$beforeSaveDraft"], ["vre"]);                                   
            ...
            if ($scope.form.$invalidEventsSummary.default
                || $scope.form.$invalidEventsSummary.blur
                || $scope.form.$invalidEventsSummary.$beforeSaveDraft) {
